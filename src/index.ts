...
const devGuild = '1056060577027457095';
...
client.on("ready", async () => {
    ...
    for (let i = 0; i < CommandFiles.length; i++) { //for runs async, foreach runs sync (at least I have had sync/async issues been fixed by changing foreach to for)
        const Command: ICommand = (await import(path.join(__dirname, 'commands', CommandFiles[i]))).default;
    ...
    client.on('interactionCreate', (interaction) => {
        ...
        const command: ICommand = commands.filter(i => i.name === interaction.data.name)[0];
        
        if (command.requiredPermissions
            && !command.requiredPermissions.every(i => interaction.memberPermissions?.has(i)))
